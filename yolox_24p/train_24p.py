...
from models import Loss_Function, prompters
from loguru import logger
from utils.lr_scheduler import cosine_lr
...
class Trainer:
    ...
    def train(self):
        ...
        if self.args.use_prompt:
            self.prompter = prompters.__dict__[args.method](args).to(self.device)
        self.max_iter = len(self.train_loader)
        self.total_steps = len(self.train_loader) * self.max_epoch
        
        self.optimizer = self.exp.get_optimizer(self.args.learn_rate)
        self.p_optimzier = torch.optim.SGD(self.prompter.parameters(),
                                         lr = args.p_lr,
                                         momentum=0.9,
                                         weight_decay=0)
        self.p_criterion = torch.nn.CrossEntropyLoss().to(self.device)
        self.p_scheduler = cosine_lr(self.p_optimzier, args.p_lr, warmup_length=self.max_epoch, steps=self.total_steps)
        ...
        for epoch in range(self.max_epoch):
            ...
            with tqdm(total = self.max_iter, desc='epoch:{}'.format(self.epoch), ncols=70) as t:
                for step, data in enumerate(self.train_loader, start=0):
                    self.current_step += 1
                    lr_step = self.total_steps + step
                    self.p_scheduler(lr_step)
                    
                    images, labels, img_info, img_id = data                

                    self.optimizer.zero_grad()
                    self.p_optimzier.zero_grad()
                    ...
                    # self.optimizer.step()
                    #p_loss = self.p_criterion(outputs, labels)
                    self.p_optimzier.step()
                    self.optimizer.step()
                ...
                self.save_model()
                if epoch % 50 == 0:
                    self.save_model(name=f'{epoch}_epoch')
                    torch.save(self.prompter, os.path.join(self.file_name, f'{epoch}_prompt.pt'))
                torch.save(self.prompter, os.path.join(self.file_name, 'prompt.pt'))
...
def make_parser():
    ...
    parser.add_argument("--use_prompt", default=False, action="store_true", help="prompt to use")
    ...
    parser.add_argument('--p_lr', type=float, default=1, help="prompt learning rate")
