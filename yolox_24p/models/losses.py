...
class IOUloss(nn.Module):
    ...
    def get_iou_24(self, pred, target):
        if pred.shape[1] != 26 or target.shape[1] != 50:
            raise IndexError
        ...
        theta = torch.tensor(15*np.pi/180, device=pred.device)
        theta_all = torch.arange(24, device=pred.device) * theta
        cos_theta_all = theta_all * torch.cos(theta_all)
        sin_theta_all = theta_all * torch.sin(theta_all)
        
        cos_theta_all = cos_theta_all.unsqueeze(0).repeat(pred.shape[0], 1)
        sin_theta_all = sin_theta_all.unsqueeze(0).repeat(pred.shape[0], 1)

        p24_x = pred[:, 2:26] * cos_theta_all + pred[:, 0].unsqueeze(1).repeat(1, 24)
        p24_y = pred[:, 2:26] * sin_theta_all + pred[:, 1].unsqueeze(1).repeat(1, 24)

        p24_x = torch.clip(p24_x, 0, 1280)
        p24_y = torch.clip(p24_y, 0, 966)
        ...
        x_min_target, y_min_target = target[:, 2::2].min(dim=1).values, target[:, 3::2].min(dim=1).values
        x_max_target, y_max_target = target[:, 2::2].max(dim=1).values, target[:, 3::2].max(dim=1).values
        gt24_rect = torch.stack([x_min_target, y_min_target, x_max_target, y_max_target], dim=0).transpose(0, 1)
        
        box_iou = self.calculate_iou(p24_rect, gt24_rect)
        ...
        # if pd_center_x < 0 or pd_center_x >= 1280 or pd_center_y < 0 or pd_center_y >= 966:
        #     iou_24, box_iou = 1000.0, 1000.0
        #     return iou_24, box_iou
        
        # if gt_center_x < 0 or gt_center_x >= 1280 or gt_center_y < 0 or gt_center_y >= 966:
        #     iou_24, box_iou = 1000.0, 1000.0
        #     return iou_24, box_iou        
...
class Loss_Function(nn.Module):
    ...
    def forward(self, outputs_train, labels):
        ...
        self.iou_24 = self.iou_loss.get_iou_24(bbox_preds.view(-1, 26)[fg_masks], reg_targets)
        # 将任务分为24个小任务
        loss_iou = (
                iou_value
            ).sum(0) / num_fg
    ...
    def pts_in_poly(self, bboxes_24p, x_idx, y_idx):
        ...
        # 计算结果的缓存容器
        is_in_boxes = torch.zeros([target_numb, pts_numb], dtype=torch.bool, device='cuda')
        # 取目标的坐标
        target_x = bboxes_24p[:,2::2]
        target_y = bboxes_24p[:,3::2]
        # 循环处理标签中的目标
        for box_idx in torch.arange(0, target_numb, 1, device='cuda'):
