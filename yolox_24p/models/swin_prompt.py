...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        B, L, C = x.shape
        H = mask_matrix.shape[0]
        W = mask_matrix.shape[1]
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, deep_prompt_embd=None):
        # print("".join(inspect.stack()[1].code_context).strip())
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            x = x.flatten(2).transpose(1, 2)  # B N C, N is H'*W'
            x = self.norm(x)
            # x = x.transpose(1, 2).view(-1, self.embed_dim, Wh, Ww)  # No need for this line if you want a 2D output
        else:
            x = x.flatten(2).transpose(1, 2)  # B N C, N is H'*W'
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=640, patch_size=4, in_chans=3, num_classes=1000,
    ...
    def forward(self, x):
        ...
        if self.ape:
            ...
            absolute_pos_embed = self.absolute_pos_embed[:, :, :H, :W].reshape(1, self.embed_dim, -1).permute(0, 2, 1)
            x = x.flatten(2).permute(0, 2, 1) + absolute_pos_embed
            x = self.pos_drop(x)
